...
class AsynchronicNeuralNetwork(NeuralNetwork):
    ...
    def do_worker(self, training_data):
        ...
        # TODO: add your code
        self.number_of_batches = len(np.arange(self.rank-self.num_masters,self.number_of_batches,self.num_workers))
        ...
        for epoch in range(self.epochs):
            ...
            for x, y in mini_batches:
                ...
                for i in range(self.num_layers):
                    self.comm.Isend(nabla_w[i],dest=i%self.num_masters,tag = )
                    self.comm.Isend(nabla_b[i], dest=i % self.num_masters)

                # recieve new self.weight and self.biases values from masters
                # TODO: add your code
                for i in range(self.num_layers):
                    self.comm.Isend(nabla_w[i], dest=i % self.num_masters)
